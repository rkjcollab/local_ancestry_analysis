...
then
   ...
   # TO NOTE: modified to set batch size to 10
   # let "nr_samples_in_batch=nr_samples/nr_batches"
   let "nr_samples_in_batch=10"
   let "nr_haplos=nr_samples_in_batch*2"

   # TO NOTE: modified to only run last set of batches
   # 54 batches of 10, 1 batch of 8 (55 batches total)
   # want to run last 58 samples that are problematic plus 10 before:
      # batches 49-55
   for ((b=49; b<=$nr_batches; b++))
   do
